...
Если $\hat{x}\in\arg\min f$ ($\hat{x}$ принадлежит множеству аргументов, при которых функция $f$ минимальна) при наложенных ограничениях - есть решения задачи, то найдётся вектор множителей Лагранжа $\lambda\in\mathbb{R}^m$ такой, что для [[Функция Лагранжа|функции Лагранжа]] $L(x;λ) = \lambda_0 f(x) + \sum_{i=1}^m\lambda_i g_i(x)$ выполняются условия:
